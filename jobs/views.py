...
def job_map_view(request):
    """Display job postings on an interactive map"""
    return render(request, 'jobs/job_map.html')


@require_http_methods(["GET"])
def job_map_api(request):
    """API endpoint to return job postings as JSON for map markers"""
    ...
    for job in jobs:
        job_data.append({
            ...
            'url': reverse('apply_to_posting', args=[job.id])
        })
